...
6. **Build release artifacts** across a matrix of native runners
   (`ubuntu-latest`, `macos-13`, `macos-latest`, `windows-latest`):
   the `krino` CLI (`-p krino --features cli --bin krino`) and the
   `krino-api` HTTP server binary, in release mode, for each target.
   Each target builds natively rather than cross-compiling, since
   `ort`'s `download-binaries` feature fetches a platform-specific
   ONNX Runtime at build time.
7. **Create a GitHub Release** with all binaries attached and the
...
|---|---|---|
| `krino-{linux-x64,macos-x64,macos-arm64,windows-x64.exe}` | GitHub Releases | CLI binary built from `-p krino --features cli`, one per target. |
| `krino-api-{linux-x64,macos-x64,macos-arm64,windows-x64.exe}` | GitHub Releases | HTTP server binary, one per target. |
...
Linux arm64 isn't built yet — no free GitHub-hosted arm64 Linux
runner was in scope when the matrix was set up. See
[docs/cli.md](cli.md#platform-support).
